  ...
  if listlevel > 1:
    ...
    for entry in msglist:
      ...
      if localsave_ready:
        ...
        filepath2write = string_sha1+'.eml'
        if not os.path.exists(filepath2write):
          with open(os.path.join(mailbox,filepath2write),'wb') as wfd:
            wfd.write(rawbuffer)
        else:
          print "DM: need not to update"
...
def main(arguments):
  ...
  if msg_count != 0 and len(flag['l']) > 0:
    pop3_worker_act(mailclient, len(flag['l']), param['m'][-1])
  # ----- -----
  # tear down
  # ----- -----
